...
const PodcastListPage = ({ route }) => {
    ...
    if (podcastList.length > 0) {
        listContents = podcastList.map(({ id, imageUrl, title, description, streamUrl }) => (
            <PodcastListItem key={Math.random()} programId={programId} id={id} imageUrl={imageUrl} title={title} description={description} streamUrl={streamUrl} />
        ))
    } else {
        listContents = (
            <RefreshHelperContainer>
